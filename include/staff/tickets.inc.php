...
switch(strtolower($_REQUEST['status'])){ //Status is overloaded
    ...
    case 'overdue':
        $status='open';
        $showoverdue=true;
        $results_type='Tickets Atrasados';
        break;
    case 'assigned':
        $status='open';
        $staffId=$thisstaff->getId();
        $results_type='Mis Tickets';
        break;
    case 'answered':
        $status='open';
        $showanswered=true;
        $results_type='Tickets Contestados';
        break;
    default:
...
if(!$results_type)
{
    ...
    else if(!strcasecmp($status,'open'))
        $results_type ='Tickets Abiertos';
    else if(!strcasecmp($status,'answered'))
        $results_type ='Tickets Contestados';
    else
...
if($search)
    $results_type.= ' (Resultado de la Busqueda)';
                ...
                <?php
                    ...
                    <th width="150">
                        <a <?php echo $assignee_sort; ?> href="tickets.php?sort=assignee&order=<?php echo $negorder; ?><?php echo $qstr; ?>"
                            title="Ordenar por Asignado <?php echo $negorder;?>">Asignado a</a></th>
                <?php
                }
            } else { ?>
                <th width="150">
                    <a <?php echo $dept_sort; ?> href="tickets.php?sort=dept&order=<?php echo $negorder;?><?php echo $qstr; ?>"
                        title="Ordenar Por Departamento <?php echo $negorder; ?>">Departamento</a></th>
            <?php
            } ?>
        </tr>
     </thead>
